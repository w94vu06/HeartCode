...
unsigned short BWRFrame0[ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 +
                         BWR_S2_SIZE];  // 心電圖區間資料For Baseline Wandering Removal (BWR) (Lead I)
unsigned short BWRFrame1[ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 +
                         BWR_S2_SIZE];  // 心電圖區間資料For Baseline Wandering Removal (BWR) (Lead II)
unsigned short BWRFrame2[ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 +
                         BWR_S2_SIZE];  // 心電圖區間資料For Baseline Wandering Removal (BWR) (Lead III)
...
unsigned short DSdownFrame[DS_FRAME_SIZE + DS_SLOPE_SIZE *
                                           2];  // The downsampled ECG buffer including two previous slope widths for the Dual-Slope QRS Detection (Lead I or Lead II)
...
void
PowerFFT();                                        // 計算心電圖片段(長度為ECGFFT_ANALY_SIZE)的Fast Fourier Transform
...
void
BWRFrame();                                        // Baseline Wandering Removal function (對一個Frame)
...
public:
	...
	****************************************************************************/
    ...
    void
    BestMorphMatch(int *newBeat, int *matchType, double *matchIndex, double *mi2, int *shiftAdj);
...
extern "C" void
fasper(float *x, float *y, unsigned long n, float ofac, float hifac, float *wk1, float *wk2,
...
Java_com_example_newidentify_MainActivity_anaEcgFile(JNIEnv *env,  jobject /* this */, jstring str,
    ...
    std::list<double>::iterator iterHRNonNNList;
...
//	strcpy ( inCHAFileName, argv[1] );		// Get the input file name from the command line argument
    ...
    strcpy ( inCHAFileName, chaFileName );
...
//    strcat(lpszFileName, ".cha");
    ...
    int preFrameAddSize = BWR_FILTER_SIZE + BWR_S2_SIZE /
                                            2;    // 前一個Frame尾端多少Size加到此Frame前(單位: unsigned short), 為了前後Frame執行BWR後的連續性
    int postFrameAddSize = BWR_FILTER_SIZE + BWR_S2_SIZE /
                                             2;    // 後一個Frame前端多少Size加到此Frame後(單位: unsigned short), 為了前後Frame執行BWR後的連續性
    ...
    char* baseName = strtok(inCHAFileName, "."); // 去除 .cha
    ...
    do {
        ...
        for (filePageN = 0; filePageN < maxPageN; filePageN++) {
            ...
            if (nBytesReceived != FileFrameSize && filePageN == maxPageN - 1) {    // 這是最後一頁
            ...
            msPerPix = (float) ECG_FRAME_SIZE / 2.0 /
                       (float) ECGDgmWidth;  // Pixels per second (x axis)
            linePix = ECGDgmWidth;
            ...
            for (fi = 0; fi < FileFrameSize / ECGCellSize; fi++) {
                ...
                if (nBytesReceived != dataCellSize * 2) {
                    ...
                    for (fi2 = 0; fi2 < dataCellSize; fi2++)
                        ECGFrame0[loadOffset +
                                  fi2] = lastECGvalue0;  // avoiding nagetive saturation
                }

                nBytesReceived = fread(ECGFrame1 + loadOffset, 1, dataCellSize * 2, pReadFile);
                if (nBytesReceived != dataCellSize * 2) {
                    for (fi2 = 0; fi2 < dataCellSize; fi2++)
                        ECGFrame1[loadOffset +
                                  fi2] = lastECGvalue1;  // avoiding nagetive saturation
                }
            ...
            BWRFrameSize = 0;
            BWRfirstPage = false;
            if (filePageN > 0) {    // 不是第一頁, 填入BWRpreFrame
                ...
                for (fi = 0; fi < FileFrameSize / ECGCellSize; fi++) {
                    nBytesReceived = fread(BWRpreFrame0 + loadOffset, 1, dataCellSize * 2,
                                           pReadFile);
                    if (nBytesReceived != dataCellSize * 2) {
                        for (fi2 = 0; fi2 < dataCellSize; fi2++)
                            BWRpreFrame0[loadOffset +
                                         fi2] = ECGADC_MIDDLE;  // avoiding nagetive saturation
                    }

                    nBytesReceived = fread(BWRpreFrame1 + loadOffset, 1, dataCellSize * 2,
                                           pReadFile);
                    if (nBytesReceived != dataCellSize * 2) {
                        for (fi2 = 0; fi2 < dataCellSize; fi2++)
                            BWRpreFrame1[loadOffset +
                                         fi2] = ECGADC_MIDDLE;  // avoiding nagetive saturation
                    }
                ...
                if (filePageN < maxPageN - 1) {    // 不是最後一頁, 將後一頁資料填入BWRpostFrame
                    ...
                    firstFail = false;
                    for (fi = 0; fi < FileFrameSize / ECGCellSize; fi++) {
                        nBytesReceived = fread(BWRpostFrame0 + loadOffset, 1, dataCellSize * 2,
                                               pReadFile);
                        if (nBytesReceived != dataCellSize * 2) {
                            ...
                            for (fi2 = 0; fi2 < dataCellSize; fi2++)
                                BWRpostFrame0[loadOffset +
                                              fi2] = lastECGvalue0;  // avoiding nagetive saturation
                        }

                        nBytesReceived = fread(BWRpostFrame1 + loadOffset, 1, dataCellSize * 2,
                                               pReadFile);
                        if (nBytesReceived != dataCellSize * 2) {
                            for (fi2 = 0; fi2 < dataCellSize; fi2++)
                                BWRpostFrame1[loadOffset +
                                              fi2] = lastECGvalue1;  // avoiding nagetive saturation
                        }
                        ...
                        unsigned short LeadIRec, LeadIIRec;
                        for (fi2 = 0; fi2 < dataCellSize; fi2++) {
                            BWRpostFrame0[loadOffset + fi2] =
                                    BWRpostFrame0[loadOffset + fi2] & 0xFFF;
                            BWRpostFrame1[loadOffset + fi2] =
                                    BWRpostFrame1[loadOffset + fi2] & 0xFFF;
                ...
                if (filePageN < maxPageN - 1) {    // 不是最後一頁, 將後一頁資料填入m_BWRpostFrame
            ...
            if (filePageN > 0) {    // Not the first frame
                for (fi = 0; fi < ECG_FRAME_SIZE / (DS_FRAME_SIZE * DS_DOWNSAMPLE_R); fi++) {
                    if (fi == 0) {
                        ...
                        for (fi2 = 0; fi2 < DS_SLOPE_SIZE * 2 * DS_DOWNSAMPLE_R; fi2++) {
                            if (fi2 % DS_DOWNSAMPLE_R == 0) {
                                if (DSdownFrameIdx < DS_FRAME_SIZE + DS_SLOPE_SIZE * 2) {
                                    if (bSelLeadII)
                                        DSdownFrame[DSdownFrameIdx] = DSpreFrame1[ECG_FRAME_SIZE -
                                                                                  DS_SLOPE_SIZE *
                                                                                  2 *
                                                                                  DS_DOWNSAMPLE_R +
                                                                                  fi2];    // Lead II
                                    else
                                        DSdownFrame[DSdownFrameIdx] = DSpreFrame0[ECG_FRAME_SIZE -
                                                                                  DS_SLOPE_SIZE *
                                                                                  2 *
                                                                                  DS_DOWNSAMPLE_R +
                                                                                  fi2];    // Lead I
                        ...
                        int DSdownFrameIdx = 0;
                        for (fi2 = 0;
                             fi2 < (DS_FRAME_SIZE + DS_SLOPE_SIZE * 2) * DS_DOWNSAMPLE_R; fi2++) {
                            if (fi2 % DS_DOWNSAMPLE_R == 0) {
                                if (DSdownFrameIdx < DS_FRAME_SIZE + DS_SLOPE_SIZE * 2) {
                                    if (bSelLeadII)
                                        DSdownFrame[DSdownFrameIdx] = ECGFrame1[fi2 +
                                                                                fi * DS_FRAME_SIZE *
                                                                                DS_DOWNSAMPLE_R -
                                                                                DS_SLOPE_SIZE * 2 *
                                                                                DS_DOWNSAMPLE_R];    // Lead II
                                    else
                                        DSdownFrame[DSdownFrameIdx] = ECGFrame0[fi2 +
                                                                                fi * DS_FRAME_SIZE *
                                                                                DS_DOWNSAMPLE_R -
                                                                                DS_SLOPE_SIZE * 2 *
                                                                                DS_DOWNSAMPLE_R];    // Lead I
                                    DSdownFrameIdx++;
                                }
                            }
                        }
                    ...
                    int tempGloRPos = 0;
                    for (iterDS_RPosList = DS_RPosList.begin();
                         iterDS_RPosList != DS_RPosList.end(); ++iterDS_RPosList) {
                        tempGloRPos = filePageN * ECG_FRAME_SIZE +
                                      (fi * DS_FRAME_SIZE - DS_FRA_ST_LEFTSH + (*iterDS_RPosList)) *
                                      DS_DOWNSAMPLE_R;
            ...
            for (fi = 0; fi < nOutCell; fi++) {    // Lead I data
                for (fi2 = 0; fi2 < dataCellSize; fi2++) {    // 將處理過的心電圖資料寫入檔案
                    ECGFrame0[prcedFrameIdx0] = ECGFrame0[prcedFrameIdx0] & 0xFFF;
                    pECGFrameOri[loadOffset + fi2] = pECGFrameOri[loadOffset + fi2] & 0xF000;
                    pECGFrameOri[loadOffset + fi2] =
                            pECGFrameOri[loadOffset + fi2] | ECGFrame0[prcedFrameIdx0];
                    prcedFrameIdx0++;
                }
                ...
                for (fi2 = 0; fi2 < dataCellSize; fi2++) {    // 將處理過的心電圖資料寫入檔案
                    ...
                    pECGFrameOri[loadOffset + dataCellSize + fi2] =
                            pECGFrameOri[loadOffset + dataCellSize + fi2] & 0xF000;
                    pECGFrameOri[loadOffset + dataCellSize + fi2] =
                            pECGFrameOri[loadOffset + dataCellSize + fi2] |
                            ECGFrame1[prcedFrameIdx1];
                    prcedFrameIdx1++;
                }

                nBytesWritten = fwrite(pECGFrameOri + loadOffset, 1, ECGCellSize, pOut1);
                if (nBytesWritten != ECGCellSize) {   // 關閉檔案
                    if (pOut1 != NULL) {
                        fclose(pOut1);
                        pOut1 = NULL;
                    }
            ...
            if (inPageN > 0) {
                ...
                LdFrameMax = ST_FRAME_FST_PAGE +
                             (LdThisPage + 1) * LD_FRAME_N_IN_PAGE;    // Currently not used.
            } else {
                LdFrameMin = 0;
            ...
            for (fi = 0; fi < allFrameSize; fi++) {    //if(fi % 5 == 4)
                {
                    // For debug
                    //m_debugInt = fi;

                    int curIdxTestR =
                            filePageN * ECG_FRAME_SIZE + fi - (DS_FRA_ST_LEFTSH * DS_DOWNSAMPLE_R);
                    if (curIdxTestR < 0)
                        continue;
                    ...
                    // Beat Classification
                    beatClasRet = aBeatClassifyForAna.BeatDetectAndClassify(intECGtemp,
                                                                            &thisBeatType,
                                                                            &thisBeatMatch);
                    if (beatClasRet) {
    ...
    fasper(HRV_HR - 1, HRV_Time - 1, HRV_Size, ofac, hifac, wk1 - 1, wk2 - 1, nwk, &nout, &jmax,
           &prob);
    ...
    double nRRsize = 0.0;
    int RRidx4UID = 0;
    for (iterRRList4UID = RRList4UID.begin();
         iterRRList4UID != RRList4UID.end(); ++iterRRList4UID) {
    ...
    if (nHRsize > 0.0) {
        ...
        double nHRNonNNsize = 0.0;
        for (iterHRNonNNList = HRList.begin(); iterHRNonNNList != HRList.end(); ++iterHRNonNNList) {
            HRmsAvg += 60000.0 / (*iterHRNonNNList);        // Unit: ms
            nHRNonNNsize += 1.0;
        }
        if (nHRNonNNsize != 0)
            HRmsAvg = HRmsAvg / nHRNonNNsize;
        else
            HRmsAvg = 0.0;

        for (iterHRNonNNList = HRList.begin(); iterHRNonNNList != HRList.end(); ++iterHRNonNNList)
            HRstdDev += (60000.0 / (*iterHRNonNNList) - HRmsAvg) *
                        (60000.0 / (*iterHRNonNNList) - HRmsAvg);    // Unit: ms

        if (nHRNonNNsize != 0)
            HRstdDev = sqrt(HRstdDev / nHRNonNNsize);
        else
            HRstdDev = 0.0;
    ...
    for (int i = 0; i < RR_Data_Len - EMBEDDING_DIMENSION * DELAY; i++)
        for (int j = i + 1; j < RR_Data_Len - EMBEDDING_DIMENSION * DELAY; j++)
            if (euclidean_distance(phase_space[i], phase_space[j], EMBEDDING_DIMENSION) < radius)
                count++;
    return (double) count / ((RR_Data_Len - EMBEDDING_DIMENSION * DELAY) *
                             ((RR_Data_Len - EMBEDDING_DIMENSION * DELAY) - 1) / 2);
}
...
void
calculate_histogram(double *intervals, int size, int *histogram, int max_bin, double bin_width) {
    if (intervals == NULL || histogram == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, "native-log", "calculate_histogram: NULL pointer passed as argument!");
        return;
    }
...
void BWRFrame() {
    ...
    unsigned short BWRbaseline0[
            ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 + BWR_S2_SIZE];  // 將BWRFrame平均算出的baseline值(Lead I)
    unsigned short BWRbaseline1[ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 +
                                BWR_S2_SIZE];  // 將BWRFrame平均算出的baseline值(Lead II)
    unsigned short BWRbaseline2[ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 +
                                BWR_S2_SIZE];  // 將BWRFrame平均算出的baseline值(Lead III)
    unsigned short BWRbaseS20[ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 +
                              BWR_S2_SIZE];  // 將BWRFrame平均算出的baseline值(stage 2) (Lead I)
    unsigned short BWRbaseS21[ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 +
                              BWR_S2_SIZE];  // 將BWRFrame平均算出的baseline值(stage 2) (Lead II)
    unsigned short BWRbaseS22[ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 +
                              BWR_S2_SIZE];  // 將BWRFrame平均算出的baseline值(stage 2) (Lead III)

    float BWR2FIRraw0[
            ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 + BWR_S2_SIZE];  // BWR後要進行FIR的buffer (Lead I)
    float BWR2FIRraw1[
            ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 + BWR_S2_SIZE];  // BWR後要進行FIR的buffer (Lead II)
    float BWR2FIRraw2[
            ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 + BWR_S2_SIZE];  // BWR後要進行FIR的buffer (Lead III)
    float BWR2FIRfilt0[
            ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 + BWR_S2_SIZE];  // BWR後要進行FIR的buffer (Lead I)
    float BWR2FIRfilt1[
            ECG_FRAME_SIZE + BWR_FILTER_SIZE * 2 + BWR_S2_SIZE];  // BWR後要進行FIR的buffer (Lead II)
    ...
    for (fi = 0; fi < BWRbufferSize; fi++) {
        BWR2FIRraw0[fi] = (float) (BWRFrame0[fi] - BWRbaseS20[fi + BWR_FILTER_SIZE / 2] +
                                   ECGADC_MIDDLE);
        BWR2FIRraw1[fi] = (float) (BWRFrame1[fi] - BWRbaseS21[fi + BWR_FILTER_SIZE / 2] +
                                   ECGADC_MIDDLE);
        BWR2FIRraw2[fi] = (float) (BWRFrame2[fi] - BWRbaseS22[fi + BWR_FILTER_SIZE / 2] +
                                   ECGADC_MIDDLE);
    }
...
void DSFrameFindR()        // The Dual-Slope QRS Detection in a DSdownFrame[] frame
{
    ...
    for (fi = 0; fi < DS_FRAME_SIZE; fi++) {
        ...
        tempSlope = ((float) (DSdownFrame[curDSFidx]) -
                     (float) (DSdownFrame[curDSFidx - DS_SLOPE_MIN])) / (float) DS_SLOPE_MIN;
        SL_max = tempSlope;
        SL_min = tempSlope;
        tempSlope = ((float) (DSdownFrame[curDSFidx]) -
                     (float) (DSdownFrame[curDSFidx + DS_SLOPE_MIN])) / (float) DS_SLOPE_MIN;
        ...
        tempRHigh =
                (float) (DSdownFrame[curDSFidx]) - (float) (DSdownFrame[curDSFidx - DS_SLOPE_MIN]);
        ...
        for (fi2 = DS_SLOPE_MIN; fi2 <= DS_SLOPE_SIZE; fi2++) {
            if (fi2 != 0)
                tempSlope = ((float) (DSdownFrame[curDSFidx]) -
                             (float) (DSdownFrame[curDSFidx - fi2])) / (float) fi2;
            else
                tempSlope = 0;
            if (tempSlope > SL_max)
                SL_max = tempSlope;
    ...
    int SDDCenterSpan = S_DIFFDIFF_HSPAN /
                        2;        // The span of the central part of the difference calculation of Sdiff_max

    float SdiffDiffBuf[DS_FRAME_SIZE + S_DIFFDIFF_HSPAN * 2 -
                       1];    // The buffer to calculate the difference of Sdiff_max including the span of the differential filter
    float S_LRminBuf[DS_FRAME_SIZE + S_DIFFDIFF_HSPAN * 2 -
                     1];    // The buffer of DS_S_LRmin[] including the starting SdiffDiff pad
    float DS_HighBuf[DS_FRAME_SIZE + S_DIFFDIFF_HSPAN * 2 -
                     1];    // The buffer of DS_High[] including the starting SdiffDiff pad
    ...
    float SdiffDiffRstBig[DS_FRAME_SIZE +
                          2];    // The buffer to detect the zero crossing of SdiffDiffRst[] including the span of the detection filter
    ...
    if (!bDS_Sdiff1stFrame) {
        ...
        for (fi = 0; fi < DS_FRAME_SIZE; fi++) {
            ...
            for (fi2 = 0; fi2 < SDDCenterSpan; fi2++) {
                tempSdiffDiff -=
                        SdiffDiffBuf[fi + fi2 + (S_DIFFDIFF_HSPAN - SDDCenterSpan)] * SDDcoefCenter;
            ...
            for (fi2 = 0; fi2 < S_DIFFDIFF_HSPAN - SDDCenterSpan; fi2++) {
                tempSdiffDiff +=
                        SdiffDiffBuf[fi + fi2 + S_DIFFDIFF_HSPAN + SDDCenterSpan] * SDDcoefSide;
            }
            if (weightCnt != 0)
            ...
            for (fi = 1; fi < DS_FRAME_SIZE + 1; fi++) {
                ...
                if (SdiffDiffRstBig[fi - 1] > 0 && SdiffDiffRstBig[fi + 1] < 0) {
                    if (SDPlastTurn == 0)  // Avoid double counting a Sdiff_max peak
                    {
                        ...
                        float fS_LRminMin = DS_S_LR_MIN_MIN /
                                            fFreqSamp;        // The min of the samller one of the left and right slopes of R point
                        ...
                        if (SdiffDiffBuf[fi - 1 - 1 + S_DIFFDIFF_HSPAN] > fCurThetaDiff) {
                            ...
                            if (SdiffAveIdx > DS_SDIFF_AVE_N) {
                                ...
                                if (S_LRminBuf[fi - 1 - 1 + S_DIFFDIFF_HSPAN] > fS_LRminMin) {
                                    ...
                                    if (DS_HighBuf[fi - 1 - 1 + S_DIFFDIFF_HSPAN] >
                                        fHighAve * DS_HIGH_AVE_RAT)
                                        //if (true)
                                    {
                                        // Criterion 3 passed
                                        ...
                                        if (lastRdistCnt > DS_LAST_R_DIST_MIN) {
                                            ...
                                            SdiffAveBuf[0] = SdiffDiffBuf[fi - 1 - 1 +
                                                                          S_DIFFDIFF_HSPAN];
                                            ...
                                            HighAveBuf[0] = DS_HighBuf[fi - 1 - 1 +
                                                                       S_DIFFDIFF_HSPAN];
                                            ...
                                            if (fSdiffAve < fSAveComp0) {
                                                fCurThetaDiff = DS_THETA_DIFF_0 / fFreqSamp;
                                            } else if (fSdiffAve >= fSAveComp0 &&
                                                       fSdiffAve < fSAveComp1) {
                                                fCurThetaDiff = DS_THETA_DIFF_1 / fFreqSamp;
                                            } else {
                                                fCurThetaDiff = DS_THETA_DIFF_2 / fFreqSamp;
                                            }
...
void notchFrame() {
    ...
    for (fi = 0; fi < iAvgN; fi++) {    //tempAvged = FIRProcessed[fi] - avg + ECGADC_MAX_F;
...
void PowerFFT() {
    ...
    MyComplex WExp[POWERFFT_SIZE_LOG +
                   1][POWERFFT_ANALY_SIZE];  // Store the precomputed complex exponentials for each stage
...
void CalPNoiseFreq() {
    ...
    int fi;
    double freqStep =
            (1000.0 / (double) POWERFFT_SAMPLE_STEP / 2.0) / ((double) POWERFFT_ANALY_SIZE / 2.0);
...
int CQRSDet::RD2lpfilt(int datum, int init) {
    ...
    y0 = (m_RD2_y1 << 1) - m_RD2_y2 + datum - (m_RD2_lpdata[halfPtr] << 1) +
         m_RD2_lpdata[m_RD2_lpptr];
    m_RD2_y2 = m_RD2_y1;
    m_RD2_y1 = y0;
...
/*	Initialize all buffers to 0 on the first call.	*/
    ...
    if (m_RD2_qpkcnt < 8) {
        ...
        if (++m_RD2_initBlank == MS1000) {
            m_RD2_initBlank = 0;
            if (m_RD2_First1s) {    // Skip the first peak
                m_RD2_First1s = 0;
    ...
    } else    /* Else test for a qrs. */
    {
        ...
        if (RD2_8to18Cnt > 0 && RD2_8to18Cnt <= FIX_R_MISS_PRD) {
            ...
            if ((m_RD2_count > m_RD2_realSbcount) &&
                (m_RD2_sbpeak > FixRMissThrsh))    // Modified by HMC and Added by HMC
            {
                memmove(&m_RD2_qrsbuf[1], m_RD2_qrsbuf, m_RD2_MEMMOVELEN);
...
void CBeatClassify::AnalyzeBeat(int *beat, int *onset, int *offset, int *isoLevel,
    ...
    if (AFfindIsoStart <= 0 &&
        AFfindIsoEnd >= BEATLGTH - AF_ISO_LENGTH) {    // No isoelectric region found
        bAF_fnd = true;
    }
...
void
CBeatClassify::PostClassify(int *recentTypes, int domType, int *recentRRs, int width, double mi2,
                            int rhythmClass) {
    ...
    if (recentTypes[1] < MAXTYPES) {
        ...
        if (((normRR - (normRR >> 3)) >= recentRRs[1]) &&
            ((recentRRs[0] - (recentRRs[0] >> 3)) >= normRR)// && (lastMI2 > 3)
...
int CBeatClassify::DomMonitor(int morphType, int rhythmClass, int beatWidth, int rr, int reset) {
    ...
    if (DMBeatCounts[dom] != 0) {
        if ((DMNormCounts[dom] == 0) ||
            (DMBeatCounts[max] / DMBeatCounts[dom] >= 2))            // == 0
        {
            HMCnewDomFnd = true;
            dom = GetDominantType();
        }
